...
**Note**: `../1_input_data/r_hiphopheads_overposted.csv` was pulled on 2019-03-31, and is likely to have been updated since.

## Initial Data
```{r Load data, eval=FALSE}
hhhe <- read_csv("../1_input_data/Reddit/r_hiphopheads_overposted.csv")
...
write_csv(artists_df, "../1_input_data/Spotify/artists.csv")
```
...
write_csv(albums_df, "../1_input_data/Spotify/albums.csv")
```
...
write_csv(tracks_df, "../1_input_data/Spotify/tracks.csv")
```
...
artists_df <- read_csv("../1_input_data/Spotify/artists.csv")
albums_df <- read_csv("../1_input_data/Spotify/albums.csv")
tracks_df <- read_csv("../1_input_data/Spotify/tracks.csv")
```
...
write_csv(artists_df_2, "../1_input_data/Spotify/artists_extended.csv")
rm(artists_df_2)
```
...
artists_df_2 <- read_csv("../1_input_data/Spotify/artists_extended_tagged.csv")
...
write_csv(albums_df_2, "../1_input_data/Spotify/albums_extended.csv")
rm(albums_df_2)
```

#### Clean Extended Albums
```{r Read in manually cleaned albums, eval=FALSE}
albums_df_2 <- read_csv("../1_input_data/Spotify/albums_extended_tagged.csv")
...
#albums_df_extended <- read_csv("../1_input_data/Spotify/albums_extended.csv") 
...
for (i in 1:length(unique(albums_df_extended$groups_to_split))){
  ...
  write_csv(x = temp,
            path =  paste0("../1_input_data/Spotify/temp_for_query/albums_df_extended_",i,".csv"))
...
write_csv(tracks_df_1, "../1_input_data/Spotify/temp_output/tracks_extended_1.csv")
```
...
write_csv(tracks_df_2, "../1_input_data/Spotify/temp_output/tracks_extended_2.csv")
```
...
write_csv(tracks_df_3, "../1_input_data/Spotify/temp_output/tracks_extended_3.csv")
```
...
write_csv(tracks_df_4, "../1_input_data/Spotify/temp_output/tracks_extended_4.csv")
```
...
write_csv(tracks_df_5, "../1_input_data/Spotify/temp_output/tracks_extended_5.csv")
```
...
write_csv(tracks_df_6, "../1_input_data/Spotify/temp_output/tracks_extended_6.csv")
```
...
write_csv(tracks_df_7, "../1_input_data/Spotify/temp_output/tracks_extended_7.csv")
```
...
write_csv(tracks_df_8, "../1_input_data/Spotify/temp_output/tracks_extended_8.csv")
```
...
write_csv(tracks_df_9, "../1_input_data/Spotify/temp_output/tracks_extended_9.csv")
```
...
write_csv(tracks_df_10, "../1_input_data/Spotify/temp_output/tracks_extended_10.csv")
```
...
# Initial Aggregation
```{r Read input data, eval=TRUE}
# Artists
artists_df <- read_csv("../1_input_data/Spotify/artists.csv")
artists_extended_df <- read_csv("../1_input_data/Spotify/artists_extended_tagged.csv")

# Albums
albums_df <- read_csv("../1_input_data/Spotify/albums.csv")
albums_extended_df <- read_csv("../1_input_data/Spotify/albums_extended_tagged.csv")

# Tracks
tracks_df <- read_csv("../1_input_data/Spotify/tracks.csv")
ext_track_files <- list.files("../1_input_data/Spotify/temp_output/")
ext_track_names <- gsub(".csv", "", ext_track_files)

for (i in 1:length(list.files("../1_input_data/Spotify/temp_output/"))){
  temp <- read_csv(paste0("../1_input_data/Spotify/temp_output/",ext_track_files[i]))
...
# 
# write_csv(tidy_tracks, "../1_input_data/tidy_tracks.csv")
...
tracks_combined <- bind_rows(tracks_df, tracks_extended_1, tracks_extended_2,
...
rm(artists_df, artists_extended_df, albums_df, albums_extended_df, 
   tracks_df, tracks_extended_1, tracks_extended_2, tracks_extended_3, 
   tracks_extended_4, tracks_extended_5, tracks_extended_6, tracks_extended_7, 
   tracks_extended_8, tracks_extended_9, tracks_extended_10, temp)
```

```{r Filter combined data to remove duplicates}
artists_combined$remove <- replace_na(artists_combined$remove, 0)
albums_combined$remove <- replace_na(albums_combined$remove, 0)
tracks_combined$remove <- replace_na(tracks_combined$remove, 0)


artists_combined <- artists_combined %>%
 filter(remove == 0)

albums_combined_test <- albums_combined %>%
  filter(remove == 0) %>%
  group_by(artist) %>%
  distinct()
...
hhh_df <- tracks_combined %>%
  left_join(albums_combined, by = "album_name") %>%
  mutate(feature = track_artists,
         feature = pmap_chr(list(feature, artist), str_remove),
         feature = str_remove(feature, "^,"),
         feature = trimws(feature, which = "both"))
         
write_csv(hhh_df, "../3_dataset/hhh_df.csv")
```
